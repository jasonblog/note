...
int CDBCtrl::SendMessageTo(CProxyMessage* pMsg)
{
    ...
    ServerInfo* dbInfo = CServerConfig::GetSingletonPtr()->GetServerInfo(
    ...
    if (iRet != 0) {
        LOG_ERROR("default",
                  "CDBCtrl::RegisterToProxyServer ConvertMsgToStream failed, iRet = {}.", iRet);
        return 0;
    }
    ...
    if (nSendReturn < 0) {
        LOG_ERROR("default", "Send Code(len:{}) To Proxy faild(error={})", nCodeLength,
                  nSendReturn);
        return -1;
    }
    ...
    const ::google::protobuf::Descriptor* pDescriptor =
        pUnknownMessagePara->GetDescriptor();
    LOG_DEBUG("default",
              "SendMessageTo: MsgName[{}] ProxyHead[{}] MsgHead[{}] MsgPara[{}]",
...
int CDBCtrl::ProcessExecuteSqlRequest(CProxyMessage* pMsg)
{
    ...
    if (pReqMsg->hasblob() == HASBLOB) {
        ...
        if (pReqMsg->bufsize() > 0) {
            if (pReqMsg->bufsize() >= MAX_PACKAGE_LEN) {
                // blob字段超出长度
                LOG_ERROR("default",
                          "Error: [{}][{}][{}], exec sql {} faild. sql_len:{} > MAX_PACKAGE_LEN\n",
                          __MY_FILE__,
                          __LINE__,
                          __FUNCTION__,
                          pReqMsg->sql().c_str(),
            ...
            int len = m_pDatabase->escape_string(sqlBuff, pReqMsg->buffer().c_str(),
                                                 pReqMsg->bufsize());
    ...
    if (MUSTCALLBACK == pReqMsg->callback()) {
        ...
        LOG_ERROR("default", "Execute SQL: {}",
                  sqlStr.c_str());    // 用于在日志中查看SQL查询语句
        ...
        if (pReqMsg->sqltype() == SELECT || pReqMsg->sqltype() == CALL) {
            ...
            if (pReqMsg->sqltype() == SELECT) {
                ...
                res = m_pDatabase->QueryForprocedure(sqlStr.c_str(), sqlStr.length(),
                                                     pReqMsg->outnumber());
            }
            ...
            if (res == NULL) { // 无返回结果
                ...
                if (res->GetRowCount() > 0) {
                    ...
                    do {
                        ...
                        if (pField == NULL) {
                            LOG_ERROR("default",
                                      "ERROR: do sql {} success, row[{}], col[{}], but some row is null\n",
                                      sqlStr.c_str(),
                                      res->GetRowCount(),
                                      res->GetFieldCount());
                            //TODO: 出错设为0
            ...
            bool bExecResult = m_pDatabase->RealDirectExecute(sqlStr.c_str(),
                               sqlStr.length());  // 执行操作
...
int CDBCtrl::ConnectToProxyServer()
{
    ...
    //如果为null 让程序崩溃
    ServerInfo* proxyInfo = CServerConfig::GetSingleton().GetServerInfo(
                                enServerType::FE_PROXYSERVER);
...
int CDBCtrl::RegisterToProxyServer(CConnector* pConnector)
{
    ...
    //如果为null 让程序崩溃
    ServerInfo* dbInfo = CServerConfig::GetSingleton().GetServerInfo(
                             enServerType::FE_DBSERVER);
    ServerInfo* proxyInfo = CServerConfig::GetSingleton().GetServerInfo(
                                enServerType::FE_PROXYSERVER);
    ...
    int iRet = CServerCommEngine::ConvertMsgToStream(&message, message_buffer,
               tTotalLen);
    ...
    if (iRet != 0) {
        LOG_ERROR("default",
                  "CDBCtrl::RegisterToProxyServer SendOneCode failed, iRet = {}.", iRet);
        return -1;
    }
...
int CDBCtrl::SendkeepAliveToProxy(CConnector* pConnector)
{
    ...
    if (iRet != 0) {
        LOG_ERROR("default",
                  "CDBCtrl::SendkeepAliveToProxy  proxy SendOneCode failed, iRet = {}.", iRet);
        return -1;
    }
...
int CDBCtrl::DispatchOneCode(int nCodeLength, BYTE* pbyCode)
{
    ...
    if (tRet != 0) {
        LOG_ERROR("default",
                  "In CDBCtrl::DispatchOneCodecode, ConvertStreamToMsg failed. tRet = {}", tRet);
        return -1;
    }
    ...
    iTempRet = Event(
                   &stTempMsg);  // 服务器执行相应的 Msg ，其实就是执行 SQL
...
int CDBCtrl::PrepareToRun()
{
    ...
    if (ConnectToProxyServer() < 0) { // 连接到proxy服务器
        LOG_ERROR("default",
                  "Error: in CDBCtrl::PrepareToRun connect proxy  server  failed!\n");
        return -1;
    }
...
void CDBCtrl::lcb_OnConnected(IBufferEvent* pBufferEvent)
{
    MY_ASSERT(pBufferEvent != NULL &&
              typeid(*pBufferEvent) == typeid(CConnector), return);
...
void CDBCtrl::lcb_OnCnsSomeDataRecv(IBufferEvent* pBufferEvent)
{
    ...
    unsigned short iTmpLen = pBufferEvent->GetRecvPackLen() - sizeof(
                                 unsigned short);
    //读取数据

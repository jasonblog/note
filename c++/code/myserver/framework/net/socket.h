...
public:
    ...
    CSocket(int32 nType = SOCK_STREAM, int32 nProtocolFamily = AF_INET,
            int32 nProtocol = 0);
    //析构函数
    ~CSocket();
    //打开
    ...
    //创建socket
    static SOCKET CreateSocket(int32 Type = SOCK_STREAM,
                               int32 ProtocolFamily = AF_INET, int32 Protocol = 0);
    //创建socket并绑定地址
    static SOCKET CreateBindedSocket(const CNetAddr& address);
    //创建非阻塞socket
    static void MakeSocketNonblocking(SOCKET Socket);
    static void Address2SockAddrIn(sockaddr_in& saiAddress,

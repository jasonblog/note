...
volatile atomic_int
g_version;  //一定要加上volatile，否則-O3會出錯，實現seqlock必要變數
...
void rd_thread(void* para)
{
    ...
    for (int i = 0; i < 1000000000; i++) {
        ...
        while (1) { //進入CS準備讀取資料
            ...
            if ((local_version = atomic_load(&g_version)) % 2 ==
                1) { //有writer正在critical section，再試一次
                retry++;
                continue;
            }

            //printf("local_version = %d, version=%d\n", local_version, atomic_load(&g_version));
            /*
            ...
            //判斷要不要重新讀取
            if (local_version == atomic_load(
                    &g_version)) {  //版本編號一樣，表示讀取過程當中，writer沒有更新資料，不用重讀
                //printf("success\n");    //for debugging
...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < nThread; i++) {
        int ret = pthread_create(&r_id[i], NULL, (void*)rd_thread,
                                 (void*)((i * 2 + 2) % 8));
        assert(ret == 0);
    }

...
static GLboolean init_opengl(void)
{
    ...
    RESOLVE_GL_FCN(PFNGLGETSHADERINFOLOGPROC, pglGetShaderInfoLog,
                   "glGetShaderInfoLog");
    ...
    RESOLVE_GL_FCN(PFNGLGETPROGRAMINFOLOGPROC, pglGetProgramInfoLog,
                   "glGetProgramInfoLog");
    RESOLVE_GL_FCN(PFNGLDELETEPROGRAMPROC, pglDeleteProgram, "glDeleteProgram");
    RESOLVE_GL_FCN(PFNGLGETUNIFORMLOCATIONPROC, pglGetUniformLocation,
                   "glGetUniformLocation");
    RESOLVE_GL_FCN(PFNGLUNIFORMMATRIX4FVPROC, pglUniformMatrix4fv,
                   "glUniformMatrix4fv");
    RESOLVE_GL_FCN(PFNGLGETATTRIBLOCATIONPROC, pglGetAttribLocation,
                   "glGetAttribLocation");
    RESOLVE_GL_FCN(PFNGLGENVERTEXARRAYSPROC, pglGenVertexArrays,
                   "glGenVertexArrays");
    RESOLVE_GL_FCN(PFNGLDELETEVERTEXARRAYSPROC, pglDeleteVertexArrays,
                   "glDeleteVertexArrays");
    RESOLVE_GL_FCN(PFNGLBINDVERTEXARRAYPROC, pglBindVertexArray,
                   "glBindVertexArray");
    ...
    RESOLVE_GL_FCN(PFNGLENABLEVERTEXATTRIBARRAYPROC, pglEnableVertexAttribArray,
                   "glEnableVertexAttribArray");
    RESOLVE_GL_FCN(PFNGLVERTEXATTRIBPOINTERPROC, pglVertexAttribPointer,
 ...
 */
static GLuint make_shader_program(const char* vertex_shader_src,
                                  const char* fragment_shader_src)
{
    ...
    vertex_shader = make_shader(GL_VERTEX_SHADER,
                                (vertex_shader_src == NULL) ? default_vertex_shader : vertex_shader_src);

    if (vertex_shader != 0u) {
        fragment_shader = make_shader(GL_FRAGMENT_SHADER,
                                      (fragment_shader_src == NULL) ? default_fragment_shader : fragment_shader_src);
...
static void make_mesh(GLuint program)
{
    ...
    pglBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(GLuint)* MAP_NUM_LINES * 2,
                  map_line_indices, GL_STATIC_DRAW);

    /* Prepare the attributes for rendering */
    attrloc = pglGetAttribLocation(program, "x");
    pglBindBuffer(GL_ARRAY_BUFFER, mesh_vbo[0]);
    pglBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * MAP_NUM_TOTAL_VERTICES,
...
{
    pglBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(GLfloat) * MAP_NUM_TOTAL_VERTICES,
                     &map_vertices[1][0]);
}
...
int main(int argc, char** argv)
{
    ...
    if (! window) {
        fprintf(stderr,
                "ERROR: Unable to create the OpenGL context and associated window\n");
        usage();

...
Logger::LoggerThread::LoggerThread(const sp<String>& name, int32_t logBufferId,
                                   bool consoleLogging, bool fileLogging, bool timestamps,
                                   int32_t priority, const sp<String>& logDirectory, const sp<String>& logFileName,
...
void Logger::LoggerThread::open()
{
    ...
    if (mFileLogging) {
        mFileHandler = new FileHandler(String::format("%s%c%s", mLogDirectory->c_str(),
                                       File::separatorChar, mLogFileName->c_str()),
...
int32_t Logger::onStartCommand(const sp<Intent>& intent, int32_t flags,
                               int32_t startId)
{
    sp<ArrayList<sp<String>>> logBuffers =
        intent->getStringArrayListExtra("logBuffers");
    ...
    if (logBuffers != nullptr) {
        if (logBuffers->contains(String::valueOf("main"))) {
            ...
            bool fileLogging = (logDirectory != nullptr && logDirectory->length() > 0 &&
                                logFileName != nullptr && logFileName->length() > 0);

            mMainLoggerThread = new LoggerThread(String::format("%s {main}", TAG),
                                                 Log::LOG_ID_MAIN, true, fileLogging, timestamps, priority,
        ...
        if (logBuffers->contains(String::valueOf("debug"))) {
            if (mDebugLoggerThread == nullptr) {
                mDebugLoggerThread = new LoggerThread(String::format("%s {debug}", TAG),
                                                      Log::LOG_ID_DEBUG, true, false, true, Log::DEBUG,
                                                      nullptr, nullptr, 0, 0);

...
//__Alignas(int)
_Alignas(int) volatile int in = 0, out = 0, num = 0;
; //新進的資料放在buffer[in]，讀取資料從buffer[out]
volatile atomic_int putWaitingOverhead = 0;
volatile atomic_int getWaitingOverhead = 0;
volatile int producerProgress = 0;
volatile int consumerProgress = 0;
...
void printBufStatus(int signo)      //每100usec印出buffer資料
{
    ...
    printf("producerProgress = %d, consumerProgress = %d\n", producerProgress,
           consumerProgress);
    printf("put overhead = %d/sec, get overhead = %d/sec\n", putWaitingOverhead,
           getWaitingOverhead);
    putWaitingOverhead = 0;
    getWaitingOverhead = 0;
}
...
void put()
{
    ...
    while ((in + 1) % bufsize == out) {
        atomic_fetch_add(&putWaitingOverhead, 1);
        ;
    }
...
void get()
{
    ...
    while (in == out) {
        atomic_fetch_add(&getWaitingOverhead, 1);
        ;
    }
    ...
    if ((tmpItem - item) != 1) {
        fprintf(stderr, "error: tmpItem - item = %d, in=%d, out=%d\n", tmpItem - item,
                in, out); //用於除錯，如果拿到的資料不是嚴格遞增，表示程式邏輯有錯
        getchar();
    }

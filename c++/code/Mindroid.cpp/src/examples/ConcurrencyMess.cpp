...
public:
    ...
    {
        Log::d(TAG, "AsyncTask1::doInBackground on thread %d with params %d",
    ...
    {
        Log::d(TAG, "AsyncTask1::onProgressUpdate on thread %d with value %d",
               (int32_t) pthread_self(), value);
    }

    virtual void onPostExecute(int32_t result)
    {
        Log::d(TAG, "AsyncTask1::onPostExecute on thread %d with result %d",
...
public:
    ...
    {
        Log::d(TAG, "AsyncTask2::doInBackground on thread %d with params %d",
    ...
    virtual void onPostExecute(int32_t result)
    {
        Log::d(TAG, "AsyncTask2::onPostExecute on thread %d with result %d",
               (int32_t) pthread_self(), result);
    }
...
int main()
{
    ...
    while (turns >= 0) {
        ...
        sHandler2->postDelayed(obtainClosure(sTestClosure1, &TestClosure1::test, 42),
                               500);

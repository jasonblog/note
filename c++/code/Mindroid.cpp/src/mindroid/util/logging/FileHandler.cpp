...
const sp<String> FileHandler::DEFAULT_PATTERN =
    String::valueOf("%h/Mindroid-%g.log");
...
void FileHandler::initOutputFiles()
{
    ...
    if (mFiles->get(0)->exists() && (!mAppend ||
                                     mFiles->get(0)->length() >= size_t(mLimit))) {
...
void FileHandler::initProperties(const sp<String>& p, bool a, int32_t l,
                                 int32_t c)
{
...
sp<String> FileHandler::parseFileName(uint32_t gen)
{
    ...
    bool homePathHasSeparatorEnd = (homePath == nullptr ? false :
                                    homePath->endsWith(File::separator));
    ...
    while ((nextChar = mPattern->indexOf('%', curChar)) >= 0) {
        if (size_t(++nextChar) < mPattern->length()) {
            ...
            case 'g':
                s = s->append(value, curChar,
                              nextChar - curChar - 1)->append(String::valueOf(gen));
                hasGeneration = true;
                break;
...
FileHandler::FileHandler(const sp<String>& pattern, int32_t limit,
                         int32_t count)
{
    if (pattern == nullptr || pattern->length() == 0) {
        Assert::fail("Pattern cannot be null or empty");

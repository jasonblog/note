...
int CS2cHandle::SendClientData(CMessage& tmpMes,
                               std::shared_ptr<CByteBuff> tmpBuff)
{
    int nTmpSocket;
...
void CS2cHandle::SendToClient(const CSocketInfo& socketInfo, const char* data,
                              unsigned int len)
{
    CAcceptor* pAcceptor = CNetWork::GetSingletonPtr()->FindAcceptor(
                               socketInfo.socketid());

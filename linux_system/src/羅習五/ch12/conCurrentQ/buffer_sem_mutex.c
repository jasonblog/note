...
long buffer[bufsize];
int in = 0, out =
             0; //新進的資料放在buffer[in]，讀取資料從buffer[out]
atomic_int
num;     //用於統計buf內共有多少資料，因為producer和consumer都可能同時修改num，因此需要宣告為atomic_int，後面會介紹
sem_t notFull, notEmpty;    //使用semaphore判斷buffer的狀態
pthread_mutex_t getMutex, putMutex;
...
void get()
{
    ...
    //printf("get %d\n", tmpItem);
    out++;
    out = out % bufsize; //規劃下次要提取資料的地方

    if ((tmpItem - item) != 1) {
        fprintf(stderr, "error: tmpItem - item = %d\n", tmpItem -
                item);    //用於除錯，如果拿到的資料不是嚴格遞增，表示程式邏輯有錯
    }

    item = tmpItem; //用於除錯，紀錄這次拿到的數字
    pthread_mutex_unlock(&getMutex);
    atomic_fetch_sub(&num,
                     1);  //讀取、減一、寫入，三個動作一次完成
...
int main(void)
{
    ...
    sem_init(&notFull, 0,
             bufsize);     //剛開始全部是空，因此有bufsize的空間可以放
    ...
    pthread_create(&id2, NULL, (void*) producer,
                   NULL);   //這個程式只支援一個producer一個consumer，拿掉這個註解會發生錯誤
    pthread_create(&id3, NULL, (void*) consumer, NULL);
    pthread_create(&id4, NULL, (void*) consumer,
                   NULL);   //這個程式只支援一個producer一個consumer，拿掉這個註解會發生錯誤

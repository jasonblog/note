...
{
    ServerInfo* rTmpProxy = CServerConfig::GetSingletonPtr()->GetServerInfo(
...
bool CServerHandle::Register2Proxy()
{
    ...
    ServerInfo
    *rTmpGame = CServerConfig::GetSingletonPtr()->GetServerInfo(
    ...
    int iRet =
        CServerCommEngine::ConvertMsgToStream(&tmpMessage, acTmpMessageBuffer,
                unTmpTotalLen);
...
bool CServerHandle::SendKeepAlive2Proxy()
{
    ...
    ServerInfo* rTmpProxy = CServerConfig::GetSingletonPtr()->GetServerInfo(
                                enServerType::FE_PROXYSERVER);
    ServerInfo* rTmpGame = CServerConfig::GetSingletonPtr()->GetServerInfo(
                               enServerType::FE_GAMESERVER);
    pbmsg_setproxy(tmpMessage.mutable_msghead(),
                   enServerType::FE_GAMESERVER,
                   rTmpGame->m_iServerId,
    ...
    int iRet = CServerCommEngine::ConvertMsgToStream(&tmpMessage,
               acTmpMessageBuffer, unTmpTotalLen);
...
bool CServerHandle::SendMessageToDB(CProxyMessage*
                                    pMsg)   //获取收到心跳的时间
{
    CProxyHead* pHead = pMsg->mutable_msghead();
    char acTmpMessageBuffer[MAX_PACKAGE_LEN];
    unsigned short unTmpTotalLen = sizeof(acTmpMessageBuffer);
    ServerInfo* gameInfo = CServerConfig::GetSingleton().GetServerInfo(
                               enServerType::FE_GAMESERVER);
    ...
    int iRet = CServerCommEngine::ConvertMsgToStream(pMsg, acTmpMessageBuffer,
               unTmpTotalLen);
    ...
    if (iRet != 0) {
        LOG_ERROR("default",
                  "[{} : {} : {}] proxy(index={}) SendOneCode failed, iRet = {}.",
    ...
    const ::google::protobuf::Descriptor* pDescriptor =
        pTmpUnknownMessagePara->GetDescriptor();
    LOG_DEBUG("default",
              "---- Send DB({}) Msg[ {} ][id: {} / {}] ----",
              pHead->dstid(),
...
void CServerHandle::lcb_OnPingServer(int fd, short what, CConnector* pConnector)
{
    ...
    ->PushTaskBack([pConnector] {
        ...
        {
            if (tNow - tmpServerHandle->GetLastSendKeepAlive() >=
...
void CServerHandle::DealServerData(IBufferEvent* pBufferEvent)
{
    ...
    if (iRet < 0) {
        LOG_ERROR("default",
                  "[{} : {} : {}] convert stream to message failed, iRet = {}.",
                  __MY_FILE__, __LINE__, __FUNCTION__, iRet);
        return;
    }
    ...
    if (tmpProxyHead.has_srcfe() && tmpProxyHead.srcfe() == FE_PROXYSERVER) {
        if (tmpProxyHead.has_srcid() &&
            tmpProxyHead.srcid() == (unsigned short) rTmpProxy->m_iServerId
            && tmpProxyHead.has_opflag() &&
            tmpProxyHead.opflag() == enMessageCmd::MESS_KEEPALIVE) {
            // 设置proxy为已连接状态
            CGameServer::GetSingletonPtr()->GetServerHandle()->SetLastRecvKeepAlive(
                GetMSTime());
        }

...
LogBuffer::LogRecord::LogRecord(const int32_t logId, const uint64_t timestamp,
                                const int32_t threadId, const int32_t priority, const sp<String>& tag,
...
{
    return String::format("%c/%s(%08x): %s", sProrities[mPriority], mTag->c_str(),
                          mThreadId, mMessage->c_str());
}
...
sp<String> LogBuffer::LogRecord::toString()
{
    ...
    return String::format("%s  %08x  %c %s: %s", time, mThreadId,
                          sProrities[mPriority], mTag->c_str(), mMessage->c_str());
}
...
bool LogBuffer::offer(const int32_t priority, const sp<String>& tag,
                      const sp<String>& message)
{
    return offer(System::currentTimeMillis(), Thread::currentThread()->getId(),
                 priority, tag, message);
}

bool LogBuffer::offer(const uint64_t timestamp, const int32_t threadId,
                      const int32_t priority, const sp<String>& tag, const sp<String>& message)
{
    ...
    uint32_t logMessageSize = TIMESTAMP_SIZE + THREAD_ID_SIZE + PRIO_SIZE + TAG_SIZE
                              + tagSize + MESSAGE_SIZE + messageSize;
    ...
    System::arraycopy(&timestamp, 0, TIMESTAMP_SIZE, logMessage, 0, logMessageSize,
                      TIMESTAMP_SIZE);
    System::arraycopy(&threadId, 0, THREAD_ID_SIZE, logMessage, TIMESTAMP_SIZE,
                      logMessageSize, THREAD_ID_SIZE);
    System::arraycopy(&priority, 0, PRIO_SIZE, logMessage,
                      TIMESTAMP_SIZE + THREAD_ID_SIZE, logMessageSize, PRIO_SIZE);
    System::arraycopy(&tagSize, 0, TAG_SIZE, logMessage,
                      TIMESTAMP_SIZE + THREAD_ID_SIZE + PRIO_SIZE, logMessageSize, TAG_SIZE);
    System::arraycopy(tag->c_str(), 0, tagSize, logMessage,
                      TIMESTAMP_SIZE + THREAD_ID_SIZE + PRIO_SIZE + TAG_SIZE, logMessageSize,
                      tagSize);
    System::arraycopy(&messageSize, 0, MESSAGE_SIZE, logMessage,
                      TIMESTAMP_SIZE + THREAD_ID_SIZE + PRIO_SIZE + TAG_SIZE + tagSize,
                      logMessageSize, MESSAGE_SIZE);
    System::arraycopy(message->c_str(), 0, messageSize, logMessage,
                      TIMESTAMP_SIZE + THREAD_ID_SIZE + PRIO_SIZE + TAG_SIZE + tagSize + MESSAGE_SIZE,
                      logMessageSize, messageSize);
...
sp<LogBuffer::LogRecord> LogBuffer::take(const int32_t minPriority)
{
    while (true) {
        ...
        int32_t priority = *reinterpret_cast<int32_t*>(record + TIMESTAMP_SIZE +
                           THREAD_ID_SIZE);
        uint32_t tagSize = *reinterpret_cast<uint32_t*>(record + TIMESTAMP_SIZE +
                           THREAD_ID_SIZE + PRIO_SIZE);
        sp<String> tag = new String(record + TIMESTAMP_SIZE + THREAD_ID_SIZE + PRIO_SIZE
                                    + TAG_SIZE, tagSize);
        uint32_t messageSize = *reinterpret_cast<uint32_t*>(record + TIMESTAMP_SIZE +
                               THREAD_ID_SIZE + PRIO_SIZE + TAG_SIZE + tagSize);
        sp<String> message = new String(record + TIMESTAMP_SIZE + THREAD_ID_SIZE +
                                        PRIO_SIZE + TAG_SIZE + tagSize + MESSAGE_SIZE, messageSize);

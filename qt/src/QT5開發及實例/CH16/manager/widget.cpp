...
void Widget::on_sellBrandComboBox_currentIndexChanged(const QString& arg1)
{
    ...
    QSqlQuery query;
    query.exec(
        QString("select price from brand where name='%1' and factory='%2'").arg(
            arg1).arg(ui->sellFactoryComboBox->currentText()));
    query.next();
    ...
    query.exec(
        QString("select last from brand where name='%1' and factory='%2'").arg(
            arg1).arg(ui->sellFactoryComboBox->currentText()));
    query.next();
...
void Widget::on_sellOkBtn_clicked()
{
    ...
    //获取以前的销售量
    query.exec(
        QString("select sell from brand where name='%1' and factory='%2'").arg(
            name).arg(factory));
    query.next();
    ...
    bool rtn = query.exec(
                   QString("update brand set sell= %1,last= %2 where name='%3' and factory='%4'").arg(
                       sell).arg(last).arg(name).arg(factory));

    if (rtn) {
        QSqlDatabase::database().commit();
        QMessageBox::information(this, tr("提示"), tr("购车成功！"),
                                 QMessageBox::Ok);
        writeXml();
        showDailyList();
        on_sellCancelBtn_clicked();
...
void Widget::createNodes(QDomElement& date)
{
    ...
    QDomText text;
    text = doc.createTextNode(QString("%1").arg(
        ...
        if (root.hasChildNodes()) {
            ...
            if (date == currentDate) {
                ...
                for (int i = 0; i < children.count(); i++) {
                    ...
                    QString str = time + "出售" + brand + factory + " " + num + "辆，" +
                                  "成交价:" + price + "万，共" + sum + "万元";
...
{
    QSqlQuery query;
    query.exec(QString("select name,sell from brand where factory='%1'").arg(
                   ui->factoryComboBox->currentText()));
    ...
    while (query.next()) {
        ...
        chartModel->setData(chartModel->index(row, 1, QModelIndex()),
                            query.value(1).toInt());
        chartModel->setData(chartModel->index(row, 0, QModelIndex()), QColor(r, g, b),
                            Qt::DecorationRole);
        row++;
    }
}

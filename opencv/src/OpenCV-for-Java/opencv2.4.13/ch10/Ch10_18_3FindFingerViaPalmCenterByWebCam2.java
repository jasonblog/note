            ...
            while (true) {
                ...
                if (!webcam_image.empty()) {
                    ...
                    if (R > 22) {
                        ...
                        Core.line(firstArea, new Point(centerX, centerY), new Point(centerX, centerY),
                                  new Scalar(255, 0, 0), 5);

                        Core.putText(firstArea, "(" + centerX + "," + centerY + ")", new Point(centerX,
                                     centerY), 0, 0.3, new Scalar(255, 0, 0)); //顯示掌心座標
                            ...
                            if (contours.size() > 0 && hull.rows() > 3) {
                                ...
                                if (tempContour.get(index2, 0) != null) {
                                    ...
                                    for (int j = 0; j < mConvexityDefectsMatOfInt4.size().height - 1 ; j++) {
                                        ...
                                        if ((pt1.x > 30 && pt1.x < 300) && (pt1.y > 90 && pt1.y < 420) &&
                                            (centerY - pt1.y > 30)) {

                                            if (CorrectPt1.isEmpty()) {
                                                CorrectPt1.add(pt1);
                                            } else {
                                                double tempDistance = getDistance(pt1,
                                                                                  (Point)CorrectPt1.get(CorrectPt1.size() - 1));
                                        ...
                                        for (int kk = 0; kk < CorrectPt1.size(); kk++) {
                                            ...
                                            Core.line(firstArea, CorrectPt1.get(kk), new Point(centerX, centerY),
                                        ...
                                        if (old_pt3 != null && old_pt1 != null) {
                                            ...
                                            Core.putText(firstArea, getAngle(pt1, old_pt3, old_pt1) + "", old_pt3, 0, 0.3,
                                                         new Scalar(0, 0, 0));//顯示指底角度
                                            double distance2 = Math.sqrt(Math.pow(pt1.x - old_pt3.x,
                                                                                  2) + Math.pow(pt1.y - old_pt3.y, 2));
                                            ...
                                            //若有角度
                                            if (getAngle(pt1, old_pt3, old_pt1) > 20 &&
                                                getAngle(pt1, old_pt3, old_pt1) < 84) {
                                                //距離太短所構成的角度不算
                                        ...
                                        if (count > 0) {
                                            System.out.println("==================總共" + ((count + 1) > 5 ? 5 :
                                                               (count + 1)) + "姆指====================="); //1~5準確
                                        } else {
                                            if (HullList.size() >= 1) {
                                                Collections.sort(HullList);

                                                if (HullList.get(HullList.size() - 1) > 80 &&
                                                    HullList.get(HullList.size() - 1) < 150) {
                                                    System.out.println(count + "==================總共0姆指=====================" +
                                                                       HullList.get(HullList.size() - 1)); //1~5準確
                                                } else if (HullList.get(HullList.size() - 1) > 150) {
                                                    System.out.println(count + "==================總共1姆指=====================" +
                                                                       HullList.get(HullList.size() - 1)); //1~5準確
                                                }

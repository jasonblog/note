...
跟蹤28979進程的所有系統調用（-e trace=all），並統計系統調用的花費時間，以及開始時間（以可視化的時分秒格式顯示），最後將記錄結果存在output.txt文件裡面。
...
對於那些著迷於底層編程細節的程序員來說，這是一個功能非常強大的工具，可用於研究編譯器和彙編器的輸出。細節信息，比如這段代碼中所顯示的這些信息，可以揭示有關本地處理器本身運行方式的很多內容。對該處理器製造商提供的技術文檔進行深入的研究，您可以收集關於一些有價值的信息，通過這些信息可以深入地瞭解內部的運行機制，因為功能程序提供了清晰的輸出。
...
Section Headers:
  ...
  ...
  ```
ELF Header 為該文件中所有段入口顯示了詳細的摘要。在列舉出這些 Header 中的內容之前，您可以看到 Header 的具體數目。在研究一個較大的目標文件時，該信息可能非常有用。

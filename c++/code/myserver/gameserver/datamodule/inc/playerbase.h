...
struct STConnectInfo {
    int
    m_iSocket;                                  // socketid即FD
    time_t
    m_tCreateTime;                              // 创建时间
    time_t
    m_tLastActiveTime;                          // 最后活跃时间
    time_t
    m_tLoginTime;                                   // 登陆时间
    ...
    void SetConnectInfo(int iSocketPara, time_t tCreateTimePara,
                        time_t tLastActiveTimePara, time_t tLoginTimePara)
    {
...
public:
    // 上次登录时间
    void SetLastLoginTime(int v)
    {
        m_iLastLoginTime = v;
    }
    int GetLastLoginTime()
    {
        return m_iLastLoginTime;
    }
    // 离线时间
    void SetLeaveTime(int v)
    {
        m_iLeaveTime = v;
    }
    int GetLeaveTime()
    ...
    int GetSocket()
    {
        return m_SocketInfo.m_iSocket;
    }
    int GetSocketCreateTime()
    {
        return  m_SocketInfo.m_tCreateTime;
    }
    int GetLastActiveTime()
    {
        return m_SocketInfo.m_tLastActiveTime;
    }
    int GetLoginTime()
    {
        return m_SocketInfo.m_tLoginTime;
    }
    // 帐号
    void SetAccount(const char* p)
    {
        ...
        strncpy(m_acAccount, p, iTmpLen >= ARRAY_CNT(m_acAccount) ? (ARRAY_CNT(
                    m_acAccount) - 1) : iTmpLen);
    }
    char* GetAccount()
    {
        return &m_acAccount[0];
    }
    // 获取连接信息
    STConnectInfo* GetSocketInfoPtr()
    {
        return &m_SocketInfo;
    }

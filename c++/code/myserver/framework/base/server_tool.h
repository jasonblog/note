...
public:
    ...
    CCoord3D static curpos(const CCoord3D& vStart, const CCoord3D& vEnd,
                           const float dis)
    {
        CCoord3D p1 = vStart;
        CCoord3D p2 = vEnd;
...
public:
    ...
    void CreateGuid(int iTime, unsigned short unZoneID, unsigned short unServerType,
                    unsigned short unServerID);
...
public:
    ...
    /*! \param rTopLeft const CWTPoint 型引用 左上角坐标
        ...
        \param vNormalize bool 型 是不是要正常化该 CWTRect 对象
        \sa CWTRect */
    CWTRect(const CWTPoint& rTopLeft, const CWTPoint& rBottomRight,
            bool vNormalize = true)
...
public:
    ...
    {
        return CWTPoint((mTopLeft.mX + mBottomRight.mX) >> 1,
                        (mTopLeft.mY + mBottomRight.mY) >> 1);
    }
    ...
    {
        return mTopLeft.mX == 0 && mTopLeft.mY == 0 && mBottomRight.mX == 0 &&
               mBottomRight.mY == 0;
    }

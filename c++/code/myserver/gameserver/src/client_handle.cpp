...
int CClientHandle::DealClientMessage(std::shared_ptr<CMessage> pMsg)
{
    ...
    if (tmpSocketInfo.state() < 0) {
        ...
        LOG_INFO("default",
                 ...
                 tmpSocketInfo.state());
        // 从连接容器中取出玩家实体
        CPlayer* pTmpTeam = CSceneObjManager::GetSingletonPtr()->GetPlayerBySocket(
                                iTmpSocket);
        ...
        if (NULL == pTmpTeam) {
            ...
            if (!connectInfo
                && (connectInfo->m_iSocket != iTmpSocket ||
                    connectInfo->m_tCreateTime != tTmpCreateTime)) {
...
int CClientHandle::SendResToPlayer(std::shared_ptr<Message> pMessage,
                                   CPlayer* pPlayer)
{
...
int CClientHandle::SendResponse(std::shared_ptr<Message> pMessage,
                                std::shared_ptr<MesHead> mesHead)
{
    ...
    iRet = mS2CPipe->AppendOneCode((BYTE*) tmpByteBuff.CanReadData(),
                                   tmpByteBuff.ReadableDataLen());

    if (iRet < 0) {
        MY_ASSERT_STR(0, return -3,
                      "CClientHandle::Send failed, AppendOneCode return {}.", iRet);
    }
...
int CClientHandle::Push(int cmd, std::shared_ptr<Message> pMessage,
                        stPointList* pTeamList)
{
    ...
    iRet = mS2CPipe->AppendOneCode((const BYTE*) tmpByteBuff.CanReadData(),
                                   tmpByteBuff.ReadableDataLen());
...
int CClientHandle::RecvClientData()
{
    ...
    int iTmpCodeLength;
    // 从共享内存管道提取消息
    int iRet = mC2SPipe->GetHeadCode((BYTE*) m_pRecvBuff->CanWriteData(),
                                     iTmpCodeLength);

    if (iRet < 0) {
        LOG_ERROR("default",
                  "[{} : {} : {}] When GetHeadCode from C2SPipe, error ocurr {}",
                                     ...
                                     time_t tCreateTime)
{
    ...
    LOG_NOTICE("default", "Disconnect Client [Socket = {} : CreateTime = {}].",
               iSocket, tCreateTime);
}

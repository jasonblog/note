...
int ConfigManage::load()
{
    ...
    if (g_areaLevelMonsterConfig.loadFromFile("area_level_monster_config.json") !=
        0) {
        return -1;
    }
    ...
    //  联盟建筑配置表
    if (g_allianceBuildingConfig.loadFromFile("alliance_building_config.json") !=
        0) {
        return -1;
    }

    //  联盟建筑数量配置表
    if (g_allianceBuildingAmountConfig.loadFromFile("alliance_building_amount_config.json")
        != 0) {
        return -1;
    }

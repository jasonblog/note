...
struct ticketlock_t {
    volatile atomic_int
    next_ticket;    //一定要加上volatile，否則-O3會出錯
    //一定要加上volatile，否則-O3會出錯，這一個不用是atomic_int，因為執行時它已經在CS
    volatile int now_serving;
};

void ticketLock_acquire(volatile atomic_int* next_ticket,
                        volatile int* now_serving)
{
    int my_ticket;
...
void thread(void* para)
{
    ...
    pid_t x = syscall(
                  __NR_gettid);     //一定要用syscall(__NR_gettid)，這一個函數印出來的thread id是Linux kernel內部使用的id，可以用來gdb
    printf("pid = %d\n", getpid());

    ...
    : QMainWindow(parent)
{
    ...
    connect(showWidget->text->document(), SIGNAL(undoAvailable(bool)), redoAction,
            SLOT(setEnabled(bool)));
    connect(showWidget->text->document(), SIGNAL(redoAvailable(bool)), redoAction,
            SLOT(setEnabled(bool)));
    connect(showWidget->text, SIGNAL(cursorPositionChanged()), this,
...
void ImgProcessor::createActions()
{
    ...
    //"打印图像"动作
    PrintImageAction = new QAction(QIcon("printImage.png"), tr("打印图像"),
                                   this);
    ...
    connect(mirrorVerticalAction, SIGNAL(triggered()), this,
            SLOT(ShowMirrorVertical()));

    //横向镜像
    mirrorHorizontalAction = new QAction(tr("横向镜像"), this);
    mirrorHorizontalAction->setStatusTip(tr("对一张图作横向镜像"));
    connect(mirrorHorizontalAction, SIGNAL(triggered()), this,
            SLOT(ShowMirrorHorizontal()));

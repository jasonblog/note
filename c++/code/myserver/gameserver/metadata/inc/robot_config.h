...
public:
    ...
    RobotCfgInfo* getCfgByID(int
                             nIndex);               //根据配置id 获取配置信息
    RobotCfgInfo*
    randRobotCfg();                       //随机获取一个配置信息
...
private:
    //初始化配置表 私用接口
    void initList(std::vector<std::string> list, std::string& name,
...
public:
    std::map<int, std::vector<RobotOnceTalkCfgInfo>>
            _mMap;  //key team; value 内容
};

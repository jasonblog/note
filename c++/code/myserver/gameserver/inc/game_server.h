...
public:
    ...
    int Push(unsigned int iMsgID, std::shared_ptr<Message> pMsgPara,
    ...
    int SendResponse(std::shared_ptr<Message> pMsgPara,
    ...
    int InitStaticLog();
    // 限制玩家登陆
    int LimitTeamLogin(unsigned int iTeamID,
...
private:
    ...
    CServerHandle*
    m_pServerHandle;             // 与服务器的连接管理(proxyserver)线程

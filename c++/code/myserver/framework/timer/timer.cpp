...
{
    if ((iIdx < 0) || (iIdx >= MAX_SESSION_PARAM_NUM) ||
        (iIdx >= m_iOtherInfoNum)) {
        return 0;
    }
...
void CSession::SetSession(time_t tMillTime, CallFunc func,
                          OBJ_ID ulOwnerEntityID, OBJ_ID ulOwnerID, bool bContinues)
{
...
{
    TIMER_FINDER::iterator tIter = m_mTimerFinder.find((pTimer->GetTimeout() /
                                   TIMER_PERCISION));
    ...
    if (tIter == m_mTimerFinder.end()) {
        ...
        tInfo.Initialize();
        tInfo.insert(pTimer);
        m_mTimerFinder.insert(TIMER_FINDER::value_type((pTimer->GetTimeout() /
                              TIMER_PERCISION), tInfo));
    } else {
                                   ...
                                   OBJ_ID ulOwnerID)
{
    ...
    // 初始化timer信息
    tpTimer->SetTimer(iTimerMark, enRunTimeMode, tMillTime, func, iCalledNum,
                      iOwnerEntityID, ulOwnerID);
...
void CTimerManager::RealDestroyTimer(OBJ_ID iObjID)
{
    ...
    if (tpItem == NULL) {
        LOG_DEBUG("default",
                  "CTimerManager::DestroyObject failed, invalid iObjID = %u.", iObjID);
        return;
    }
    ...
    // 销毁实体
    LOG_DEBUG("default", "[{} : {} : {}] DeleteObject.", __MY_FILE__, __LINE__,
              __FUNCTION__);
    DeleteObject(iObjID);
}
...
int CTimerManager::CheckTimerQueue(time_t tNow)
{
    ...
    while (nowTick > m_tLastCheckTick) {
        ...
        // 循环遍历定时器,处理超时
        for (auto tIter = m_mTimerFinder.begin(); tIter != m_mTimerFinder.end();
             tIter++) {
...
CTimerBase* CTimerManager::Timeout(CTimerBase* pTimer, time_t tNow, bool bClear)
{
    ...
    if (((pTimer->GetTimerType() == ETT_TIMER) && (bClear == false) &&
         ((((CTimer*) pTimer))->GetCalledNum() != 0))
        || (pTimer->GetTimerType() == ETT_SESSION) &&
        (((CSession*) pTimer)->IsContinues() == true)) {
        // 将该结点插入新的链表
...
// 创建session
CSession* CTimerManager::CreateSession(time_t tMillTime,
                                       CSession::CallFunc func, int iOwnerEntityID, OBJ_ID ulOwnerID)
{
...
CObj* CTimerManager::CreateObject(ETimerType eType)
{
    ...
    default: {
            LOG_ERROR("default",
                      "CTimerManager::CreateObject failed, object type({}) invalid.", eType);
            break;
        }
    }
...
int CTimerManager::DeleteObject(OBJ_ID iObjID)
{
    ...
    default: {
            LOG_ERROR("default",
                      "CTimerManager::DeleteObject failed, object id = %u, type = {}.", iObjID,
                      tType);
            return -1;
        }
    }

    LOG_INFO("default", "DeleteObject Type({}) ObjID %u.", GetTimerTypeName(tType),
             iObjID);

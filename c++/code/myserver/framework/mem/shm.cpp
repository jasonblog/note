...
CSharedMem::CSharedMem(unsigned int nKey, size_t nSize)
{
    ...
    //计算crc校验值
    nTempInt =
        (size_t) m_nShmKey ^ (size_t) m_nShmSize ^ (size_t) m_tCreateTime ^
        (size_t) m_tLastStamp ^ (size_t) m_nCRC;
...
int CSharedMem::Initialize(unsigned int nKey, size_t nSize)
{
    ...
    m_nCRC = (size_t) m_nShmKey ^ (size_t) m_nShmSize ^ (size_t) m_tCreateTime ^
             (size_t) m_tLastStamp;

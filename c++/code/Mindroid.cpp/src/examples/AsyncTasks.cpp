...
public:
    ...
    {
        Log::d("ExampleAsyncTask", "onPreExecute on thread %d",
               (int32_t) pthread_self());
    }

    virtual int32_t doInBackground(int32_t param)
    {
        Log::d("ExampleAsyncTask", "doInBackground on thread %d with param %d",
    ...
    {
        Log::d("ExampleAsyncTask", "onProgressUpdate on thread %d with value %d",
               (int32_t) pthread_self(), value);
    }

    virtual void onPostExecute(int32_t result)
    {
        Log::d("ExampleAsyncTask", "onPostExecute on thread %d with result %d",
               (int32_t) pthread_self(), result);
    }

    virtual void onCancelled()
    {
        Log::d("ExampleAsyncTask", "onCancelled on thread %d",
               (int32_t) pthread_self());
    }
};

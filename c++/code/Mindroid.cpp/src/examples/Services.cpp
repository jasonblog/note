...
public:
    ...
    public:
        ...
        {
            Log::d(TAG, "Service1::onServiceDisconnected: %s",
                   name->toShortString()->c_str());
        }
    };
...
void startSystemSerices(sp<IServiceManager> serviceManager)
{
    ...
    ->putStringArrayListExtra("logBuffers",
                              getLogBuffers(String::valueOf("main, events, debug")))
    ...
    packageManager->setComponent(new ComponentName("mindroid",
...
{
    sp<IPackageManager> packageManager = binder::PackageManager::Stub::asInterface(
            ServiceManager::getSystemService(Context::PACKAGE_MANAGER));
    sp<ArrayList<sp<PackageInfo>>> packages = packageManager->getInstalledPackages(

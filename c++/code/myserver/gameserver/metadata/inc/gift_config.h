...
public:
    ...
    GiftCfgInfo*    getCfg(int
                           nIndex);                                     //根据配置id 获取配置信息
    GiftTypeList*   getIdByType(int
                                nType);                                 //根据礼包档位 获取配置ID列表
    int             makeTypeByCfgID(int
                                    nIndex);                            //根据配置id 生成一个档位key 必须保证nIndex 的合法性 不然就返回-1
    int             makeTypeByCfgID(int rechargeIOS,
                                    int rechargeAndroid);  //根据配置的充值档id 生成一个档位key
    bool            initRate(std::map<int, int>& mRate,
                             int nRate);         //用_datatype 初始化一个权重表
    int
    getMaxType();                                           //获取最高充值档档key
    int             getNextType(int
                                nType);                                 //获取下一档充值档key 不做最高充值档判断

private:
    std::map<int, GiftCfgInfo*>     _datas;             //ID = 配置信息
    std::map<int, GiftTypeList*>
    _datatype;          //礼包档位 ＝ 对应的礼包id 列表
    int                             _maxtype;           //最高充值档

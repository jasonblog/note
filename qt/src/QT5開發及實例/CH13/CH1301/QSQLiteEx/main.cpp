...
int main(int argc, char* argv[])
{
    ...
    QSqlQuery query;
    bool success =
        query.exec("create table automobil(id int primary key,attribute varchar,type varchar,kind varchar,nation int,carnumber int,elevaltor int,distance int,oil int,temperature int)");
    ...
    qDebug() << QObject::tr("插入 %1 条记录，耗时：%2 ms").arg(
                 records).arg(t.elapsed());
    ...
    if (success) {
        qDebug() << QObject::tr("排序 %1 条记录，耗时：%2 ms").arg(
                     records).arg(t.elapsed());
    } else {
    ...
    qDebug() << QObject::tr("更新 %1 条记录，耗时：%2 ms").arg(
                 records).arg(t.elapsed());

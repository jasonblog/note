...
int CCodeQueue::GetCanReadLen()
{
    if (m_stQueueHead.m_iReadIndex == m_stQueueHead.m_iWriteIndex) {
        ...
        return (m_stQueueHead.m_iSize - (m_stQueueHead.m_iReadIndex -
                                         m_stQueueHead.m_iWriteIndex));
    }
}
...
int CCodeQueue::GetCanWriteLen()
{
    ...
    if (m_stQueueHead.m_iReadIndex == m_stQueueHead.m_iWriteIndex) {
        ...
        nTempMaxLength = (m_stQueueHead.m_iSize - m_stQueueHead.m_iWriteIndex) +
                         m_stQueueHead.m_iReadIndex;
    }
...
int CCodeQueue::AppendOneCode(const BYTE* pInCode, int sInLength)
{
    ...
    //可用内存在共享内存的中间连续区域
    if (nTempRead > nTempWrite) {
        memcpy((void*) &pbyCodeBuf[nTempWrite], (const void*) pInCode,
               (size_t) sInLength);
    }
    //可用内存在共享内存的两头
    else {
        ...
        if (sInLength > (m_stQueueHead.m_iSize - nTempWrite)) {
            ...
            memcpy((void*) &pbyCodeBuf[0],
                   (const void*) &pInCode[(m_stQueueHead.m_iSize - nTempWrite)],
                   (size_t)(sInLength - (m_stQueueHead.m_iSize - nTempWrite)));
        }
        //内存区尾部够够长不需要分片
        else {
            memcpy((void*) &pbyCodeBuf[nTempWrite], (const void*) &pInCode[0],
                   (size_t) sInLength);
        }
    }
...
int CCodeQueue::PeekHeadCode(BYTE* pOutCode, int& psOutLength)
{
    ...
    //数据在内存去中间
    if (nTempRead < nTempWrite) {
        memcpy((void*) pTempDst, (const void*) &pTempSrc[nTempRead],
               (size_t)(psOutLength));
    }
    //数据在内存区两头
    else {
...
/**
  ...
  * */
int CCodeQueue::GetOneCode(int iCodeOffset, int nCodeLength, BYTE* pOutCode,
                           int& psOutLength)
{
    short nTempShort = 0;
    int nTempMaxLength = 0;
    ...
    if (nTempShort > (int)(nTempMaxLength - sizeof(short)) || nTempShort < 0 ||
        nTempShort != nCodeLength) {
        psOutLength = 0;
        return -2;
    }
...
int CCodeQueue::DumpToFile(const char* szFileName)
{
    ...
    fwrite((const void*) pPage, m_stQueueHead.m_iSize - iPageSize * iPageCount, 1,
           fpDumpFile);
...
int CCodeQueue::LoadFromFile(const char* szFileName)
{
    ...
    fread((void*) pPage, m_stQueueHead.m_iSize - iPageSize * iPageCount, 1,
          fpDumpFile);

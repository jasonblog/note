                   ...
                   unsigned int vMaxBackupIndex)           /*回卷文件个数*/
{
    ...
    auto log = spdlog::stdout_logger_mt(vLogName);
#else
    auto log = spdlog::rotating_logger_mt(vLogName, vLogDir, vMaxFileSize,
                                          vMaxBackupIndex);
#endif
#else
#ifdef _DEBUG_
    auto log = spdlog::stdout_logger_st(vLogName);
#else
    auto log = spdlog::rotating_logger_st(vLogName, vLogDir, vMaxFileSize,
                                          vMaxBackupIndex);
#endif
#endif

...
BYTE* CreateShareMem(key_t iKey, long vSize)
{
    ...
    if (iShmID < 0) {
        ...
        //            exit(-1);
        //        }
        LOG_INFO("default", "Same shm seg (key={}) exist, now try to attach it...",
                 iKey);

        iShmID = shmget(iKey, iTempShmSize, 0666);

        if (iShmID < 0) {
            LOG_INFO("default", "Attach to share memory {} failed, {}. Now try to touch it",
            ...
            if (iShmID < 0) {
                ...
                if (iShmID < 0) {
                    LOG_ERROR("default", "Fatal error, alloc share memory failed, {}",
                              strerror(errno));
                    exit(-1);
                }
            }
    ...
    LOG_INFO("default",
             "Successfully alloced share memory block, (key={}), id = {}, size = {}",
             iKey, iShmID, iTempShmSize);
    BYTE* tpShm = (BYTE*) shmat(iShmID, NULL, 0);

    if ((void*) -1 == tpShm) {
        LOG_ERROR("default",
                  "create share memory failed, shmat failed, iShmID = {}, error = {}.",
                  iShmID, strerror(errno));
        exit(0);
    }
...
int DestroyShareMem(key_t iKey)
{
    ...
    if (iShmID < 0) {
        ...
        LOG_INFO("default", "Remove shared memory(id = {}, key = {}) succeed.", iShmID,
                 iKey);
    }

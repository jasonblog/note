...
int CGameServer::Push(unsigned int iMsgID, std::shared_ptr<Message> pMsgPara,
                      stPointList* pTeamList)
{
    GetIoThread()->PushTaskBack(std::mem_fn(&CClientHandle::Push), m_pClientHandle,
                                iMsgID, pMsgPara, pTeamList);
    return 0;
}
                      ...
                      CPlayer* pPlayer)
{
    ...
    GetIoThread()->PushTaskBack(std::mem_fn(&CClientHandle::Push), m_pClientHandle,
                                iMsgID, pMsgPara, &tmpList);
    return 0;
}

// 回复客户端上行的请求
int CGameServer::SendResponse(std::shared_ptr<Message> pMsgPara,
                              CPlayer* pPlayer)
{
    MY_ASSERT(pPlayer != NULL && pMsgPara != NULL,
              return -1);
    GetIoThread()->PushTaskBack(std::mem_fn(&CClientHandle::SendResToPlayer),
                                m_pClientHandle, pMsgPara, pPlayer);
    return 0;
}
                              ...
                              std::shared_ptr<MesHead> mesHead)
{
    ...
    GetIoThread()->PushTaskBack(std::mem_fn(&CClientHandle::SendResponse),
                                m_pClientHandle, pMsgPara, mesHead);
}
...
void CGameServer::LoadDataFinish()
{
    ...
    LOG_INFO("default", "[{} : {} : {}] Launch server load data finished.",
...
// 限制玩家登陆
int CGameServer::LimitTeamLogin(unsigned int iTeamID,
                                time_t iTimes)// itimes 暂定为小时
{
    return 0;
}

...
void rd_thread(void* para)
{
    ...
    for (int i = 0; i < 1000000000; i++) {
        ...
        while (1) { //進入CS準備讀取資料
            ...
            if (atomic_load_explicit(&g_version,
                                     memory_order_acquire) % 2 == 1) { //有writer正在critical section，再試一次
                retry++;
                continue;
            }
            ...
            //判斷要不要重新讀取
            if (init_version == atomic_load_explicit(&g_version,
                    memory_order_acquire)) { //版本編號一樣，表示讀取過程當中，writer沒有更新資料，不用重讀
                //printf("success\n");    //for debugging

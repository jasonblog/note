...
bool CC2sHandle::BeginListen()
{
    ...
    if (iRet) {
        LOG_INFO("default", "Server listen success at {} : {}",
                 gateInfo->m_sHost.c_str(), gateInfo->m_iPort);
        return true;
    } else {
        exit(0);
    }
...
void CC2sHandle::ClearSocket(IBufferEvent* pAcceptor, short iError)
{
    ...
    CNetWork::GetSingletonPtr()->ShutDownAcceptor(
...
void CC2sHandle::DisConnect(IBufferEvent* pAcceptor, short iError)
{
    ...
    iRet = m_pC2SPipe->AppendOneCode((BYTE*) m_pSendBuff->CanReadData(),
                                     m_pSendBuff->ReadableDataLen());
...
void CC2sHandle::SendToGame(IBufferEvent* pAcceptor, unsigned short tmpLastLen)
{
    ...
    if (0 == iTmpRet && tmpHead.cmd() != 103 && tmpLastLen >= 0) {
        ...
        //发送给game server
        iTmpRet = m_pC2SPipe->AppendOneCode((const BYTE*) m_pSendBuff->CanReadData(),
                                            m_pSendBuff->ReadableDataLen());

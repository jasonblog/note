...
void MainWindow::createActions()        //创建主窗体的所有动作
{
    ...
    connect(addEllipseItemAct, SIGNAL(triggered()), this,
            SLOT(slotAddEllipseItem()));
    connect(addPolygonItemAct, SIGNAL(triggered()), this,
            SLOT(slotAddPolygonItem()));
    ...
    connect(addAnimItemAct, SIGNAL(triggered()), this,
            SLOT(slotAddAnimationItem()));
}
...
{
    QVector<QPoint> v;
    v << QPoint(30, -15) << QPoint(0, -30) << QPoint(-30, -15) << QPoint(-30,
            15) << QPoint(0, 30) << QPoint(30, 15);

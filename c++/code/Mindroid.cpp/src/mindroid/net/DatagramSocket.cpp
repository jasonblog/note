...
bool DatagramSocket::bind(uint16_t port)
{
    ...
    if (socketAddress->getInetAddress() != nullptr && mSocketId != -1) {
        mIsBound = ::bind(mSocketId, socketAddress->getInetAddress()->getPointer(),
...
{
    return ::recvfrom(mSocketId, reinterpret_cast<char*>(data), size, 0, nullptr,
                      0);
}

ssize_t DatagramSocket::recv(uint8_t* data, size_t size,
                             sp<SocketAddress>& sender)
{
    ssize_t result = 0;
...
bool DatagramSocket::send(const void* data, size_t size,
                          const sp<SocketAddress>& receiver)
{
    if (receiver->getInetAddress() != nullptr) {
        if (mSocketId == -1) {
            mSocketId = ::socket(receiver->getInetAddress()->isInet6Address() ? AF_INET6 :
                                 AF_INET, SOCK_DGRAM, 0);
        }

        socklen_t socketSize = receiver->getInetAddress()->getSize();
        return (size_t) ::sendto(mSocketId, reinterpret_cast<const char*>(data), size,
                                 0,

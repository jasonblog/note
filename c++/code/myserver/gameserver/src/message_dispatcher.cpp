...
template<> CMessageDispatcher* CSingleton<CMessageDispatcher>::spSingleton =
    NULL;
...
int CMessageDispatcher::ProcessClientMessage(std::shared_ptr<CMessage> pMsg)
{
    ...
    const ::google::protobuf::Descriptor* pTmpDescriptor =
        pMsgPara->GetDescriptor();

    if (tmpHead.cmd() == PlayerCommandId::USER_ACCOUNT_LOGIN) {
        PERF_FUNC(pTmpDescriptor->name().c_str(),
                  CCoreModule::GetSingletonPtr()->OnMsgUserLoginRequest(pMsg.get()));
    } else if (tmpHead.cmd() == PlayerCommandId::PLAYER_LOGIN) {
        PERF_FUNC(pTmpDescriptor->name().c_str(),
                  CCoreModule::GetSingletonPtr()->OnMsgPlayerLoginRequest(pMsg.get()));
    } else {
        CSocketInfo tmpSocketInfo = tmpHead.socketinfos().Get(0);
        // 获得CTeam 实体
        CPlayer* pPlayer = CSceneObjManager::GetSingletonPtr()->GetPlayer(
                               tmpSocketInfo.socketid());

        if (NULL == pPlayer) {
            // 未找到玩家实体
            LOG_ERROR("default",
                      "[{} : {} : {}] ProcessClientMsg failed, Invalid socket ({}).",
...
int CMessageDispatcher::ProcessServerMessage(CProxyMessage* pMsg)
{
    ...
    PERF_FUNC(pTmpDescriptor->name().c_str(),
              CGameServer::GetSingletonPtr()->ProcessRouterMessage(pMsg));

...
void Widget::usrEnter(QString usrname, QString ipaddr)
{
    ...
    if (isEmpty) {
        ...
        ui->usrNumLbl->setText(tr("在线人数：%1").arg(
                                   ui->usrTblWidget->rowCount()));
...
{
    int rowNum = ui->usrTblWidget->findItems(usrname,
...
void Widget::on_sendTBtn_clicked()
{
    if (ui->usrTblWidget->selectedItems().isEmpty()) {
        QMessageBox::warning(0, tr("选择用户"), tr("请先选择目标用户！"),
                             QMessageBox::Ok);
        return;
    }
                            ...
                            QString filename)
{
    ...
    if (ipAddr == clntaddr) {
        int btn = QMessageBox::information(this, tr("接受文件"),
                                           tr("来自%1(%2)的文件：%3,是否接收？").arg(usrname).arg(srvaddr).arg(
...
void Widget::curFmtChanged(const QTextCharFormat& fmt)
{
    ...
    if (fmt.fontPointSize() < 8) {
        ...
        ui->sizeCbx->setCurrentIndex(ui->sizeCbx->findText(QString::number(
                                         fmt.fontPointSize())));
    }

...
public:
    ...
    WeakReferenceImpl(Object* object) :
        ...
        mMemorizeRefOperationsDuringRefTracking(
            !!DEBUG_REFERENCES_MEMORIZE_REF_OPERATIONS_DURING_REF_TRACKING)
    {
    }
    ...
    ~WeakReferenceImpl()
    {
        if (!mMemorizeRefOperationsDuringRefTracking && mStrongReferences != nullptr) {
            ...
            while (item) {
                ...
                DEBUG_INFO("\t%c ID %p (reference couter is %d):", c, item->id,
        ...
        if (!mMemorizeRefOperationsDuringRefTracking && mWeakReferences != nullptr) {
            ...
            while (item) {
                char c = item->referenceCounter >= 0 ? '+' : '-';
                DEBUG_INFO("\t%c ID %p (reference counter is %d):", c, item->id,
                           item->referenceCounter);
                item = item->nextItem;
            }
        }
    ...
    void trackReference(bool trackReferences,
...
Object::~Object()
{
    if ((mReference->mFlags & OBJECT_LIFETIME_MASK) ==
        OBJECT_LIFETIME_WEAK_REFERENCE) {
...
void Object::incStrongReference(const void* id) const
{
    ...
    const int32_t oldStrongReferenceCount = AtomicInteger::getAndIncrement(
            &reference->mStrongReferenceCounter);
    ASSERT(oldStrongReferenceCount > 0,
           "Object::incStrongReference() called on %p after underflow", reference);
#if PRINT_REFERENCES
    ...
    AtomicInteger::getAndAdd(-INITIAL_STRONG_REFERENCE_VALUE,
                             &reference->mStrongReferenceCounter);
...
void Object::decStrongReference(const void* id) const
{
    ...
    const int32_t oldStrongReferenceCount = AtomicInteger::getAndDecrement(
            &reference->mStrongReferenceCounter);
#if PRINT_REFERENCES
    DEBUG_INFO("Object::decStrongReference() of %p from %p: reference count is %d\n",
               this, id, oldStrongReferenceCount - 1);
#endif
    ASSERT(oldStrongReferenceCount >= 1,
           "Object::decStrongReference() called on %p too many times", reference);

    if (oldStrongReferenceCount == 1) {
        const_cast<Object*>(this)->onLastReference(id);

        if ((reference->mFlags & OBJECT_LIFETIME_MASK) ==
            OBJECT_LIFETIME_STRONG_REFERENCE) {
...
void Object::WeakReference::incWeakReference(const void* id)
{
    ...
    const int32_t oldWeakReferenceCount = AtomicInteger::getAndIncrement(
            &reference->mWeakReferenceCounter);
    ASSERT(oldWeakReferenceCount >= 0,
           "Object::WeakReference::incWeakReference() called on %p after underflow", this);
}
...
void Object::WeakReference::decWeakReference(const void* id)
{
    ...
    const int32_t oldWeakReferenceCount = AtomicInteger::getAndDecrement(
            &reference->mWeakReferenceCounter);
    ASSERT(oldWeakReferenceCount >= 1,
           "Object::WeakReference::decWeakReference() called on %p too many times", this);
...
bool Object::WeakReference::tryIncStrongReference(const void* id)
{
    ...
    ASSERT(strongReferenceCount >= 0,
           "Object::WeakReference::tryIncStrongReference() called on %p after underflow",
           this);

    while (strongReferenceCount > 0 &&
           strongReferenceCount != INITIAL_STRONG_REFERENCE_VALUE) {
        if (AtomicInteger::compareAndSwap(strongReferenceCount,
                                          strongReferenceCount + 1, &reference->mStrongReferenceCounter) == 0) {
            break;
        }
    ...
    if (strongReferenceCount <= 0 ||
        ...
        if ((reference->mFlags & OBJECT_LIFETIME_MASK) ==
            ...
            strongReferenceCount = AtomicInteger::getAndIncrement(
                                       &reference->mStrongReferenceCounter);
        }
    }
    ...
    while (strongReferenceCount >= INITIAL_STRONG_REFERENCE_VALUE) {
        ASSERT(strongReferenceCount > INITIAL_STRONG_REFERENCE_VALUE,
               "Object::WeakReference::tryIncStrongReference() of %p underflowed INITIAL_STRONG_REFERENCE_VALUE",
               this);

        if (AtomicInteger::compareAndSwap(strongReferenceCount,
                                          strongReferenceCount - INITIAL_STRONG_REFERENCE_VALUE,
                                          &reference->mStrongReferenceCounter) == 0) {
            break;
        }
...
void Object::WeakReference::trackReference(bool trackReference,
        bool memorizeRefOperationsDuringRefTracking)
{
    static_cast<WeakReferenceImpl*>(this)->trackReference(trackReference,
            memorizeRefOperationsDuringRefTracking);
}

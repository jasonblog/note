 ...
 */
const sp<String::StringBuffer> String::EMPTY_STRING_BUFFER = new
...
bool String::equals(const sp<String>& string) const
{
    if (mStringBuffer->mData && string->mStringBuffer->mData) {
        ...
        if (mStringBuffer->mData == nullptr &&
            string->mStringBuffer->mData == nullptr) {
            return true;
        } else {
            return false;
        }
...
sp<String> String::format(const char* format, ...)
{
    ...
    sp<String> formattedString = EMPTY_STRING->appendFormattedWithVarArgList(format,
                                 args);
    va_end(args);
...
sp<String> String::replace(char oldChar, char newChar)
{
    ...
    for (size_t i = 0; i < length(); i++) {
        stringBuffer->mData[i] = (stringBuffer->mData[i] == oldChar) ? newChar :
                                 stringBuffer->mData[i];
    }
...
sp<String> String::append(const char* string, size_t size) const
{
    if (string != nullptr && size > 0) {
        return new String(new StringBuffer(mStringBuffer->mData, mStringBuffer->mSize,
                                           string, size));
    }
...
sp<String> String::appendFormattedWithVarArgList(const char* format,
        va_list args) const
{

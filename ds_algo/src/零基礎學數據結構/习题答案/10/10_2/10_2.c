...
void Dfs(AdjGraph G, int v, int* vexnum, int* arcnum);
int LocateVertex(AdjGraph G, VertexType v)
/*返回图中顶点对应的位置*/
{
    int i;
...
/*判断无向图G是否是一棵树*/
{
    int i, vexnum = 0, arcnum = 0;

    for (i = 0; i < G.vexnum; i++) { /*初始时，所有结点都标志为未访问*/
        visited[i] = 0;
    }

    Dfs(G, 0, &vexnum, &arcnum); /*深度遍历图G，并统计访问过的顶点数目和边的数目*/

    if (vexnum == G.vexnum &&
        arcnum == 2 * (G.vexnum -
                       1)) { /*如果访问到的顶点数目等于图G的顶点数，且访问边的数目等于2*(G.vexnum-1)则说明是一棵树*/
        return 1;
    } else {
        return 0;
    }

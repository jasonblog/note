...
int CModuleManager::RegisterModule(EModuleType eType, CLogicModule* pModule)
{
    if (pModule == NULL) {
        LOG_FATAL("default",
                  "[{} : {} : {}] RegisterModule failed, eType({}) pModule == NULL.",
                  __MY_FILE__, __LINE__, __FUNCTION__, eType);
        return -1;
    }

    if (eType <= EModuleType::EMODULETYPE_INVALID ||
        eType >= EModuleType::EMODULETYPE_NUM) {
...
void CModuleManager::OnClientMessage(int iModuleType, CPlayer* pTeam,
                                     CMessage* pMsg)
{
    //    MY_ASSERT(pMsg->has_msghead() == true, return);
    if (iModuleType < EMODULETYPE_START || iModuleType >= EMODULETYPE_NUM ||

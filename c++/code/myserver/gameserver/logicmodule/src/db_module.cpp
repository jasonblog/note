...
void CDbModule::OnMsgExecuteSqlResponse(CProxyMessage* pMsg)
{
    ...
    if (pTmpSqlRsp->sessionid() != 0) {
        // 如果有session
        pTmpSession =
            (CSession*) CGameServer::GetSingletonPtr()->GetTimerManager()->GetObject(
                pTmpSqlRsp->sessionid());

        if (NULL == pTmpSession) {
            LOG_ERROR("db",
                      "[{} : {} : {}] Session doesn't exsit (logictype:{}:sessionid:%u ), has been deleted.",
        ...
        if (pTmpSession->GetTimeout() != (time_t) pTmpSqlRsp->timestamp()) {
            LOG_ERROR("db",
                      "[{} : {} : {}] Session doesn't exsit (logictype:{}:sessionid:%u ), has been reused.",
                      __MY_FILE__, __LINE__, __FUNCTION__,
                      pTmpSqlRsp->logictype(), pTmpSqlRsp->sessionid());
            return;
        }
                          ...
                          const char* pSql, ...)
{
    ...
    int iTmpLen = ::vsnprintf(acTmpSqlCommond, sizeof(acTmpSqlCommond), pSql,
                              tmpArgs);
    ...
    LOG_DEBUG("db", "[{}]", ((Message*)
                             tmpMsg.msgpara())->ShortDebugString().c_str());
...
int CDbModule::ExecuteSqlForBlob(emDBLogicType nLogicType,
                                 ...
                                 MesHead* mesHead, ...)
{
    MY_ASSERT_LOG("db", pSql != NULL && pBlob != NULL &&
                  pSQLWhere != NULL, return -1);
...
void CDbModule::FindOrCreateUserRequest(const std::string& platform,
                                        const std::string& puid, MesHead* mesHead)
{
    char* pcTmpSql = (char*)
                     "SELECT `player_id` FROM user WHERE `platform` = {} AND `puid` = {}";
...
void CDbModule::FindOrCreateUserResponse(CSession* pSession,
        CMsgExecuteSqlResponse* pMsgSql, CProxyHead* proxyHead)
{
    MY_ASSERT_LOG("db", pMsgSql != NULL && pSession != NULL &&
                  proxyHead != NULL, return);
    ...
    if (1 == pMsgSql->resultcode()) {
        ...
        if (0 == iTmpRowCount || 0 == iTmpColCount) {
            player_id = (OBJ_ID)
                        CSceneObjManager::GetSingletonPtr()->GetPlayerManager()->GetValidId();
        } else {
            // 表示有返回结果
        ...
        CGameServer::GetSingletonPtr()->SendResponse(std::shared_ptr<Message>(pRes),
                std::shared_ptr<MesHead>(pHead));
    } else {
        // 拉取数据失败,则退出
        LOG_ERROR("db",
                  "[{} : {} : {}] LoadAllAccountResponse failed, resultcode={}, row={}, col={}.",
                  __MY_FILE__, __LINE__, __FUNCTION__, pMsgSql->resultcode(), iTmpRowCount,
                  iTmpColCount);
        return;
    }
}
...
CSocket::CSocket(int32 nType/*=SOCK_STREAM*/, int32 nProtocolFamily/*=AF_INET*/,
                 int32 nProtocol/*=0*/)
...
SOCKET CSocket::CreateSocket(int32 Type, int32 ProtocolFamily, int32 Protocol)
{
    ...
    if (Socket == INVALID_SOCKET) {
        MY_ASSERT_STR(false, return INVALID_SOCKET,
                      "socket failed with error code %d .", PpeGetLastError());
    }
    ...
    if (SOCKET_ERROR ==
        setsockopt(Socket, SOL_SOCKET, SO_REUSEADDR,
                   reinterpret_cast<const char*>(&iVal), sizeof(iVal))) {
        MY_ASSERT_STR(false, return INVALID_SOCKET,
                      "setsockopt failed with error code %d .", PpeGetLastError());
    }
...
int CSocket::GetSocketError() const
{
    ...
    default:
        MY_ASSERT_STR(false, DO_NOTHING, "unknown failed!!");
    }
...
uint32 CSocket::Bind(const CNetAddr& addr)
{
    ...
    //bind
    if (bind(m_Socket, reinterpret_cast<sockaddr*>(&saiAddress),
             sizeof(sockaddr))) {
        CloseSocket(m_Socket);
        m_Socket = INVALID_SOCKET;
...
//--------------------------------- *tools* -------------------------------------------------------------------
void CSocket::Address2SockAddrIn(sockaddr_in& saiAddress,
...
void CSocket::MakeSocketNonblocking(SOCKET Socket)
{
    if (-1 == evutil_make_socket_nonblocking(static_cast<int>(Socket))) {
        CloseSocket(Socket);
        MY_ASSERT_STR(false, return, "ioctlsocket failed with error code %d.",
                      PpeGetLastError());
    }
}
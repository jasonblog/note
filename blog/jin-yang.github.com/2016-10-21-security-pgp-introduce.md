...
<!--
    ...
    3.1 公開密鑰管理機制
    一個成熟的加密體系必然要有一個成熟的密鑰管理機制配磁。公鑰體制的提出就是為瞭解決傳統加密體系的密鑰分配過程不安全、不方便的缺點。例如網絡黑客們常用的手段之一就是“監聽”，通過網絡傳送的密鑰很容易被截獲。對PGP來說，公鑰本來就是要公開，就沒有防監聽的問題。但公鑰的發佈仍然可能存在安全性問題，例如公鑰被篡改（public key tampering），使得使用公鑰與公鑰持有人的公鑰不一致。這在公鑰密碼體系中是很嚴重的安全問題。因此必須幫助用戶確信使用的公鑰是與他通信的對方的公鑰。

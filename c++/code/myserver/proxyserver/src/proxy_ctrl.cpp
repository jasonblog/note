...
int CProxyCtrl::PrepareToRun()
{
    ...
    ServerInfo* proxyInfo = CServerConfig::GetSingletonPtr()->GetServerInfo(
...
int CProxyCtrl::Run()
{
    LOG_INFO("default", "Libevent run with net module {}",
             event_base_get_method(reinterpret_cast<const event_base*>
                                   (CNetWork::GetSingletonPtr()
                                    ->GetEventReactor()->GetEventBase())));
    ServerInfo* proxyInfo = CServerConfig::GetSingletonPtr()->GetServerInfo(
                                enServerType::FE_PROXYSERVER);
    LOG_INFO("default", "ProxyServer is going to run at {} : {}",
...
void CProxyCtrl::lcb_OnAcceptCns(uint32 uId, IBufferEvent* pBufferEvent)
{
    MY_ASSERT(pBufferEvent != NULL &&
              typeid(*pBufferEvent) == typeid(CAcceptor), return);
    CNetWork::GetSingletonPtr()->InsertNewAcceptor(uId, (CAcceptor*) pBufferEvent);
    LOG_DEBUG("default", "New Connector,socket id {}",
...
void CProxyCtrl::lcb_OnCnsDisconnected(IBufferEvent* pBufferEvent)
{
    ...
    LOG_WARN("default", "Connection disconnected,socket id {}",
             pBufferEvent->GetSocket().GetSocket());
...
void CProxyCtrl::lcb_OnAcceptorTimeOut(CAcceptor* pAcceptor)
{
    ...
    LOG_INFO("default", "Connection time out,socket id {} close",
             pAcceptor->GetSocket().GetSocket());
}

int CProxyCtrl::DealRegisterMes(IBufferEvent* pBufferEvent,
                                unsigned short iTmpLen)
{
    MY_ASSERT(pBufferEvent != NULL, return -1);

    CProxyHead stTmpProxyHead;
    int iRet = CServerCommEngine::ConvertStreamToProxy(m_acRecvBuff + sizeof(
                   unsigned short), iTmpLen, &stTmpProxyHead);

    if (iRet < 0) {
        LOG_ERROR("default", "In DealRegisterMes, ConvertStreamToProxy return {}.",
                  iRet);
        return -1;
    }
...
/**
  ...
  *功能描述        : 转发一个数据包
**/
int CProxyCtrl::TransferOneCode(IBufferEvent* pBufferEvent,
                                unsigned short nCodeLength)
{
    ...
    int iRet = CServerCommEngine::ConvertStreamToMsg(m_acRecvBuff + sizeof(
                   unsigned short), nCodeLength, &stTmpMessage);

    if (iRet < 0) {
        LOG_ERROR("default", "In TransferOneCode, ConvertStreamToCSHead return {}.",
                  iRet);
        return -1;
    }
    ...
    LOG_INFO("default",
             "Transfer code begin, from(FE = {} : ID = {}) to(FE = {} : ID = {}), timestamp = {}",
    ...
    if (stTmpHead.dstfe() == FE_PROXYSERVER) {
        ...
        case enMessageCmd::MESS_KEEPALIVE: {
                ...
                ServerInfo* serverInfo = CServerConfig::GetSingletonPtr()->GetServerInfo(
                                             enServerType::FE_PROXYSERVER);
                ...
                int iRet = CServerCommEngine::ConvertMsgToStream(&stRetMessage, message_buffer,
                           tTotalLen);

                if (iRet != 0) {
                    LOG_ERROR("default",
                              "CDBCtrsl::SendkeepAliveToProxy ConvertMsgToStream failed, iRet = {}.", iRet);
                    return 0;
                } else {
                    ...
                    if (iRet != 0) {
                        LOG_INFO("default",
                                 "send keepalive to (FE = {} : ID = {}), SendOneCodeTo failed, iRet = {}.",
    ...
    if (iTempRet) {
        LOG_INFO("default",
                 "transfer one code from (FE = {} : ID = {}) to (FE = {} : ID = {}) failed of {}.",
                 stTmpHead.srcfe(), stTmpHead.srcid(), stTmpHead.dstfe(), stTmpHead.dstid(),
                 iTempRet);
        m_stStatLog.iSndCnt--;
        m_stStatLog.iSndSize -= nCodeLength;
    }
...
int CProxyCtrl::SendOneCodeTo(short nCodeLength, BYTE* pbyCode, int iKey,

        ...
        MesHead* pHead)
{
    if ((byteBuff == NULL) || (pHead == NULL)) {
        MY_ASSERT_STR(0, return -1,
                      "CClientCommEngine::ParseClientStream Input param failed.");
    }
        ...
        MesHead* pHead)
{
    if ((convertBuff == NULL) || (pDataBuff == NULL) || pHead == NULL) {
        MY_ASSERT_STR(0, return -1,
                      "CClientCommEngine::ConverGateToGame Input param failed.");
    }
    ...
    // MesHead
    if (pHead->SerializeToArray(convertBuff->CanWriteData(),
                                convertBuff->WriteableDataLen()) == false) {
        MY_ASSERT_STR(0, return -2,
                      "CClientCommEngine::ConvertMsgToStream CTcpHead SerializeToArray failed.");
    }
        ...
        Message* pMsg)
{
    if (convertBuff == NULL || pHead == NULL) {
        MY_ASSERT_STR(0, return -1,
                      "CClientCommEngine::ConvertMsgToStream Input param failed.");
    }
    ...
    // MesHead
    if (pHead->SerializeToArray(convertBuff->CanWriteData(),
                                convertBuff->WriteableDataLen() - unLength) == false) {
        MY_ASSERT_STR(0, return -1,
                      "CClientCommEngine::ConvertToGateStream CTcpHead SerializeToArray failed.");
    }
    ...
    if (pMsg) {
        ...
        if (pMsg->SerializeToArray(tpEncryBuff, iMsgParaLen) == false) {
            MY_ASSERT_STR(0, return -1,
                          "CClientCommEngine::ConvertToGateStream MsgPara SerializeToArray failed.");
        }
        ...
        unsigned short seq)
{
    ...
    // MesHead
    if (pHead->SerializeToArray(pBuff->CanWriteData(),
                                pBuff->WriteableDataLen()) == false) {
        MY_ASSERT_STR(0, return -1,
                      "CClientCommEngine::ConvertToGateStream CTcpHead SerializeToArray failed.");
    }
    ...
    int dataLen = sizeof(unsigned short) + sizeof(cmd) + sizeof(serial) + sizeof(
                      seq) + pMsg->GetCachedSize();
    ...
    if (pMsg->SerializeToArray(pBuff->CanWriteData(),
                               pBuff->WriteableDataLen()) == false) {
        MY_ASSERT_STR(0, return -1,
                      "CClientCommEngine::ConvertToGateStream MsgPara SerializeToArray failed.");
    }
        ...
        CFactory* pMsgFactory)
{
    if ((pBuff == NULL)) {
        MY_ASSERT_STR(0, return -1,
                      "CClientCommEngine::ConvertStreamToClientMsg Input param failed.");
    }
    ...
    //序列化消息,消息
    if ((unTmpTotalLen - unTmpUseLen) > 0 && pMsgFactory != NULL &&
        pMessage != NULL) {
        // MessagePara
        // 使用消息工厂

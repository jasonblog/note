...
void Process::Stub::onTransact(int32_t what, int32_t arg1, int32_t arg2,
...
void Process::Stub::Proxy::createService(const sp<Intent>& intent,
...
void Process::Stub::Proxy::startService(const sp<Intent>& intent, int32_t flags,
                                        int32_t startId, const sp<IRemoteCallback>& callback)
{
    sp<Bundle> data = new Bundle();
    data->putObject("intent", intent);
    data->putBinder("binder", callback->asBinder());
    mRemote->transact(MSG_START_SERVICE, flags, startId, data, nullptr,
                      FLAG_ONEWAY);
}
...
void Process::Stub::Proxy::bindService(const sp<Intent>& intent,
...
void Process::Stub::SmartProxy::createService(const sp<Intent>& intent,
        const sp<IRemoteCallback>& callback)
{
...
void Process::Stub::SmartProxy::startService(const sp<Intent>& intent,
        int32_t flags, int32_t startId, const sp<IRemoteCallback>& callback)
{
    if (mRemote->runsOnSameThread()) {
        mStub->startService(intent, flags, startId,
                            RemoteCallback::Stub::asInterface(callback->asBinder()));
    } else {
...
void Process::Stub::SmartProxy::bindService(const sp<Intent>& intent,
        const sp<ServiceConnection>& conn, int32_t flags,
        const sp<IRemoteCallback>& callback)
{
    if (mRemote->runsOnSameThread()) {
        mStub->bindService(intent, conn, flags,
                           RemoteCallback::Stub::asInterface(callback->asBinder()));
    } else {

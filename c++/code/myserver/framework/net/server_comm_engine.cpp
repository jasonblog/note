        ...
        CFactory* pMsgFactory)
{
    if ((pBuff == NULL) || (pMsg == NULL)) {
        MY_ASSERT_STR(0, return -1,
                      "CServerCommEngine::ConvertStreamToMsg Input param failed.");
    }
    ...
    if (tAddLen >= 8) {
        MY_ASSERT_STR(0, return -3,
                      "CServerCommEngine::ConvertStreamToMsg tAddLen = %d impossibility.", tAddLen);
    }
    ...
    if (pProxyHead->ParseFromArray(tpBuff, tTmpLen) != true) {
        MY_ASSERT_STR(0, return -4,
                      "CServerCommEngine::ConvertStreamToMsg CProxyHead ParseFromArray failed.");
    }
    ...
    if (tTotalLen < 0) {
        MY_ASSERT_STR(0, return -5,
                      "CServerCommEngine::ConvertStreamToMsg tTotalLen = %d impossibility.",
                      tTotalLen);
    }
        ...
        unsigned short& unBuffLen)
{
    if ((pBuff == NULL) || (unBuffLen < 8)) {
        MY_ASSERT_STR(0, return -1,
                      "CServerCommEngine::ConvertMsgToStream Input impossibility.");
    }
    ...
    *(unsigned short*) tpBuff = pProxyHead->ByteSize();
    ...
    if (pProxyHead->SerializeToArray(tpBuff, unBuffLen - tLen) == false) {
        MY_ASSERT_STR(0, return -2,
                      "CServerCommEngine::ConvertMsgToStream CProxyHead SerializeToArray failed.");
    }
    ...
    if (tpMsgPara == NULL) {
        MY_ASSERT_STR(0, return -4,
                      "CServerCommEngine::ConvertMsgToStream CMessagePara is NULL.");
    }
    ...
    *(unsigned short*) tpBuff = tpMsgPara->ByteSize();
    ...
    if (tpMsgPara->SerializeToArray(tpBuff, unBuffLen - tLen) == false) {
        MY_ASSERT_STR(0, return -5,
                      "CServerCommEngine::ConvertMsgToStream msgpara SerializeToArray failed.");
    }
...
{
    if ((pBuff == NULL) || (unBuffLen < (sizeof(unsigned short) * 3)) ||
        (pProxyHead == NULL)) {
        MY_ASSERT_STR(0, return -1,
                      "CServerCommEngine::ConvertStreamToMsg Input param failed.");
    }
    ...
    if (tAddLen >= 8) {
        MY_ASSERT_STR(0, return -3,
                      "CServerCommEngine::ConvertStreamToMsg tAddLen = %d impoosibility.", tAddLen);
    }

...
BakeMuttonCommand::BakeMuttonCommand(Barbecuer* barbecuer): Command(
        barbecuer) {}
...
BakeChickenCommand::BakeChickenCommand(Barbecuer* barbecuer): Command(
        barbecuer) {}
...
void Waiter::SetOrder(Command* command)
{
    if (dynamic_cast<BakeChickenCommand*>(command)) {
        ...
        std::cout << "add: " << std::string(typeid(*command).name()).substr(
                      2) << std::endl;
    }
}
...
void Waiter::CancelOrder(Command* command)
{
    for (std::vector <Command*> ::iterator it = commands_.begin();
        ...
        if (*it == command) {
            commands_.erase(it);
            std::cout << "cancel: " << std::string(typeid(*command).name()).substr(
                          2) << std::endl;
            return;
        }
    }

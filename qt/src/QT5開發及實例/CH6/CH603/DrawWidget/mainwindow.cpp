    ...
    : QMainWindow(parent)
{
    ...
    setCentralWidget(
        drawWidget);   //新建的DrawWidget对象作为主窗口的中央窗体
...
void MainWindow::createToolBar()
{
    ...
    connect(styleComboBox, SIGNAL(activated(int)), this,
            SLOT(ShowStyle())); //关联相应的槽函数

    widthLabel = new QLabel(tr("线宽："));           //创建线宽选择控件
    widthSpinBox = new QSpinBox;
    connect(widthSpinBox, SIGNAL(valueChanged(int)), drawWidget,
            SLOT(setWidth(int)));
...
void MainWindow::ShowColor()
{
    QColor color = QColorDialog::getColor(static_cast<int>(Qt::black),
                                          this);   //使用标准颜色对话框QColorDialog获得一个颜色值
